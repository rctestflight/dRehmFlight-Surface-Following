...
#include <PWMServo.h> //Commanding any extra actuators, installed with teensyduino installers
#include "src/NickPing/NickPing.h" // Daniel's ultrasonic altitude sensor
...
float GyroErrorX = -1.35;
float GyroErrorY = -0.81;
...
float Ki_roll_angle = 1;    //Roll I-gain - angle mode
float Kd_roll_angle = 0;   //Roll D-gain - angle mode (has no effect on controlANGLE2)
float B_loop_roll = 0.9;      //Roll damping term for controlANGLE2(), lower is more damping (must be between 0 to 1)
float Kp_pitch_angle = 3;   //Pitch P-gain - angle mode
float Ki_pitch_angle = 1;   //Pitch I-gain - angle mode
...
float alt_setpoint = 50;   //Desired altitude, cm 
float kP_alt = 2;           //Altitude Hold P-gain
float kI_alt = 1;          //Altitude Hold I-gain
float kD_alt = 0.00;          //Altitude Hold D-gain
bool dual_measurement = true; // If true, combines sonar1 and sonar2 measurements for altitude estimation. If false, only uses sonar1 for altitude estimation
...
//Sonar
const int sonar1Trig = 15;
const int sonar1Echo = 16;
const int sonar2Trig = 17;
const int sonar2Echo = 20;
...
float alt_integral, alt_prev, alt_PID = 0.0;
float sonar1_filtered, sonar2_filtered = 0.0;
bool sonar_toggle = false;
...
void controlMixer() {
   ...
   *alt_PID - stabilized altitude variable
  ...
  */
  ...
  //Hydrofoil Testbed Mixing 

 if (channel_5_pwm < 1500) {  //disarmed
  s5_command_scaled = 0; //Front Left
  s6_command_scaled = 0; //Front Right
  s1_command_scaled = 0.52; // Left
  s2_command_scaled = 0.5; // Right
  s3_command_scaled = 0.5; // Left
  s4_command_scaled = 0.51; //Right
  }
  
 if (channel_6_pwm < 1500 && channel_5_pwm > 1500) { 
  s5_command_scaled = thro_des + yaw_passthru; //Front Left
  s6_command_scaled = thro_des - yaw_passthru; //Front Right

  s1_command_scaled = 0.52 + roll_passthru; // Left
  s2_command_scaled = 0.5 + roll_passthru; // Right
  s3_command_scaled = 0.5 + pitch_passthru; // Left
  s4_command_scaled = 0.51 - pitch_passthru; //Right

  }
 
  if (channel_6_pwm > 1500 && channel_5_pwm > 1500) { 

  // s5_command_scaled = thro_des + alt_PID + yaw_PID; //Front Left  
  // s6_command_scaled = thro_des + alt_PID - yaw_PID; //Front Right  

  s5_command_scaled = thro_des + yaw_PID; //Front Left
  s6_command_scaled = thro_des - yaw_PID; //Front Right

  s1_command_scaled = 0.52 + roll_PID;
  s2_command_scaled = 0.5 + roll_PID;
  s3_command_scaled = 0.5 + pitch_PID;// + alt_PID;
  s4_command_scaled = 0.51 - pitch_PID;// + alt_PID; 
  }
...
void controlAlt() {
  ...
  float alt_error_max = 50.0; //cm
  float alt_beta = 0.01;

/* Origional:
  //get the distance data
  if (sonar2.pulse_started == false && sonar_toggle == false)
  {
    sonar1.update(1);
    sonar_toggle = true;
  }
  
  else if (sonar1.pulse_started == false)
  {
    sonar2.update(2); //comment this out to turn off sonar 2
    sonar_toggle = false;
  }
  */ //I made this new one in an attempt to make the sonars stop interfering with eachother:

  //get the distance data
  if (sonar2.pulse_started == false && sonar1Delay > 10)//&& sonar_toggle == false)
  {
    sonar1.update(1);
    sonar_toggle = true;
    sonar2Delay = millis();
  ...
  if (sonar1.pulse_started == false && sonar2Delay > 10)
  {
    sonar2.update(2); //comment this out to turn off sonar 2
    sonar_toggle = false;
    sonar1Delay = millis();
  ...
  float sonar1_raw = sonar1.dist_cm;
  float sonar2_raw = sonar2.dist_cm;
   /*
  Serial.print(F("sonar1: "));
  Serial.print(sonar1_raw);
  Serial.print(F(" sonar2: "));
  Serial.println(sonar2_raw);
   */
  ...
  ang_from_level = 0; //turning off angle compensation
  sonar1_filtered = alt_beta*sonar1_raw * cos(ang_from_level * 0.0174533) + (1.0 - alt_beta)*sonar1_filtered;
  sonar2_filtered = alt_beta*sonar2_raw * cos(ang_from_level * 0.0174533) + (1.0 - alt_beta)*sonar2_filtered;
  */

  sonar1_filtered = sonar1_raw; // Daniel made this. Its bypassing all the angle stuff
  sonar2_filtered = sonar2_raw;

  //combine the two
  float alt = 0.0;
  if (dual_measurement == true)
  {
      alt = (sonar1_filtered + sonar2_filtered) / 2.0;
  }
  else
  {
      alt = sonar1_filtered;
  ...
  //Serial.println( alt ); //Serial Print combined sonar altitude

  //get derivative
  float vel = (alt - alt_prev)/dt;
  alt_prev = alt;
  //Serial.println(vel);

  //PID controller for altitude
  float error_alt = alt_setpoint - alt;
  error_alt = constrain(error_alt, -alt_error_max, alt_error_max);
  alt_integral = alt_integral + error_alt*dt;
  if (channel_5_pwm > 1500) {   //Don't let integrator build if disarmed
    alt_integral = 0.0; 
  }
  alt_integral = constrain(alt_integral, -alt_int_max, alt_int_max); //saturate integral term to prevent wind up
  alt_PID = 0.01*(kP_alt*error_alt + kI_alt*alt_integral - kD_alt*vel); //stabilized variable for altitude
  alt_PID = constrain(alt_PID, -0.5, 0.5);
...
void sonar1_isr()
{
  sonar1.processInterrupt();
}

void sonar2_isr()
{
  sonar2.processInterrupt();
}
...
void printSonarData() {
  if (current_time - print_counter > 10000) {
    print_counter = micros();
  //Serial.print(F(" sonar1_filtered: "));
  //Serial.println(sonar1_filtered);
  //Serial.print(F(" sonar2_filtered: "));
  //Serial.print(sonar2_filtered);
